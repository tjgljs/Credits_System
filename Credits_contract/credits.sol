    ...
    contract Creditcertification is Ownable{
        ...
        //学生授权第三方允许查询自己学分详情
        function approve(address spender,uint256 num, uint256 requestId )external returns(bool){
        
            allowance[msg.sender][spender]=num;

            for (uint i = 0; i < requestOfMechanism[msg.sender].length; i++) {
            if (requestOfMechanism[msg.sender][i].id == requestId) {
                requestOfMechanism[msg.sender][i].isApproved = true;
                break;
            }
        }
