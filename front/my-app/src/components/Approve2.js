...
const ApproveCredits = () => {
  ...
  const approveTransfer = async (mechanismAddr, num) => {
    console.log("1",mechanismAddr,num)
    if (!contract) {
    ...
    try {
      const tx = await contract.approve(mechanismAddr, num);
      await tx.wait();
...
<List
    ...
    renderItem={(item, index) => (
        <List.Item actions={[
            <Button onClick={() => approveTransfer(item.mechanismAddr, item.num)} disabled={item.isApproved}>批准</Button>,
        ]}>
            <List.Item.Meta
