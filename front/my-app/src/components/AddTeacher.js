...
const AddTeacher = () => {
    ...
    const handleTransfer = async () => {
        ...
        try {
            const tx = await contract.addTeacher(teacherAddr);
            await tx.wait();
            message.success('Add teacher successful');

            try{
                const token=localStorage.getItem('token');

                const response=await axios.post('http://localhost:8080/admin/add_teacher',{
                ...
                console.log(response.data);
                message.success('Teacher added successfully in backed');
            }catch(error){
    ...
    return (
        <div>
            <h2 style={h2Style}>Add Teacher</h2>
