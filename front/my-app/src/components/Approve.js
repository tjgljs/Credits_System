...
const ApproveCredits = () => {
    ...
    const handleApprove = async () => {
        ...
        try {
            const tx = await contract.approve(spender, num);
            await tx.wait();
    ...
    return (
        <div>
            <h2 style={h2Style}>Please approve third-party companies and institutions to query your credit details</h2>
            
            <Form form={form} name="validateOnly" layout="vertical" autoComplete="off" onFinish={handleApprove}>
            <Form.Item name="spender" label="Spender"  rules={[{ required: true }]} >
                <Input value={spender} onChange={e => setSpender(e.target.value)}/>
            </Form.Item>
            <Form.Item name="num" label="Num" rules={[{ required: true }]} >
